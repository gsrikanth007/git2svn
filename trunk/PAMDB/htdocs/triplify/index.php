 ...
 * @version    $Id: index.php 56 2009-09-28 14:26:29Z seebi $
 */
...
if($_SERVER['REQUEST_URI']) {
	...
	if($argv[1])
		include($argv[1]);
	$_SERVER['REQUEST_URI']=$baseURI='http://linkedgeodata.org/triplify/';
...
echo "<rdf:RDF ";
while (list($ns, $uri) = each($triplify['namespaces'])) {
	if ($ns == 'vocabulary')
		printf(" xmlns=\"%s\"", $uri);
	else 
		printf(" xmlns:%s=\"%s\"", $ns, $uri);
}
  echo ">\n";
...
class tripleizer {
	...
	function tripleize($queries,$c=NULL,$id=NULL) {
		...
		$this->writeTriple($self,'rdfs:comment','Generated by Triplify '.$this->version.' (http://Triplify.org)',true);
		if($this->config['license'])
			$this->writeTriple($self,'cc:license',$this->config['license']);
		if(is_array($this->config['metadata']))
			foreach($this->config['metadata'] as $property=>$value) if($value)
				$this->writeTriple($self,$property,$value,true);
		
		if($this->curr_subject != NULL) echo "</rdf:Description>\n";
		$this->curr_subject = NULL;
		foreach($queries as $class=>$q) if(!$c || $class==$c || (substr($class,0,1)=='/' && preg_match($class,$GLOBALS['path']))) {
			...
			foreach(is_array($q)?$q:array($q) as $qid=>$query) {
				...
				if(substr($class,0,1)=='/') {
					...
					$this->writeTriple($this->uri($class),'rdf:type',$this->uri('owl:Class'));
					continue(2);
				} else
...
#echo $query;
	...
	function makeTriples($cl,$class,$maketype,$dtypes) {
		...
		if(!$GLOBALS['triplify']['noType'] && $maketype && substr($class,0,1)!='/') {
			...
			$this->writeTriple($subject,'rdf:type',$this->uri($c,$this->ns['vocabulary']));
		}
		#foreach($cl as $p=>$val)
		reset($cl);
	...
	function writeTriple($subject,$predicate,$object,$isLiteral=false,$dtype=NULL,$lang=NULL) {
		...
		if($_REQUEST['t-output']=='json') {
			...
			if($this->curr_subject != $subject) {
				if($this->curr_subject != NULL) echo "</rdf:Description>\n";
				$this->curr_subject = $subject;
				echo "<rdf:Description rdf:about=\"$subject\">\n";
			}
			if($isLiteral) {
				$object=$this->xml_escape($object);
				echo '  <'.$predicate.($lang?' xml:lang="'.$lang.'"':'').'>'.$object.'</'.$predicate.">\n";
			}
			else
				echo '  <'.$predicate.' rdf:resource="'.$object."\"/>\n";
		}
		flush();
	}
	/**
